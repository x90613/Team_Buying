...
public class HostFormDAO {
  ...
  private static final String INSERT_SQL =
      "INSERT INTO host_form (host_id, menu_id, title, store_name, description, status, start_time,"
          + " dead_time, host_contact_information, transfer_information, participant_information,"
          + " link, open, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

  private static final String UPDATE_SQL =
      "UPDATE host_form SET host_id = ?, menu_id = ?, title = ?, store_name = ?, description = ?,"
          + " status = ?, start_time = ?, dead_time = ?, host_contact_information = ?,"
          + " transfer_information = ?, participant_information = ?, link = ?, open = ?, image = ?"
          + " WHERE id = ?";

  private static final String SELECT_BY_ID_SQL = "SELECT * FROM host_form WHERE id = ?";

  private static final String SELECT_BY_HOST_ID_SQL = "SELECT * FROM host_form WHERE host_id = ?";

  private static final String DELETE_SQL = "DELETE FROM host_form WHERE id = ?";
  ...
  private HostForm update(HostForm hostForm) {
    ...
    if (updated == 0) {
      ...
      throw new RuntimeException(
          "No rows affected when updating host form with id: " + hostForm.getId());
    }
  ...
  public Optional<HostForm> findById(Integer id) {
    try {
      ...
      HostForm hostForm = jdbcTemplate.queryForObject(SELECT_BY_ID_SQL, this::mapRowToHostForm, id);
  ...
  public List<HostForm> findByHostId(Integer hostId) {
    try {
      ...
      List<HostForm> forms =
          jdbcTemplate.query(SELECT_BY_HOST_ID_SQL, this::mapRowToHostForm, hostId);
  ...
  public void delete(HostForm hostForm) {
    try {
      ...
      if (deleted == 0) {
        ...
        throw new RuntimeException(
            "No rows deleted when deleting host form with id: " + hostForm.getId());
      }
  ...
  public boolean existsByIdAndHostId(Integer id, Integer hostId) {
    try {
      ...
      Integer count =
          jdbcTemplate.queryForObject(EXISTS_BY_ID_AND_HOST_ID_SQL, Integer.class, id, hostId);
