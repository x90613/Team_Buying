...
public class HostFormService {
  ...
  public HostForm createHostForm(CreateHostFormDTO request) {
    try {
      ...
      // 設置聯絡資訊
      hostForm.setHostContactInformation(
          request.getHostContactInformation() != null
              ? request.getHostContactInformation().trim()
              : null);
      hostForm.setTransferInformation(
          request.getTransferInformation() != null
              ? request.getTransferInformation().trim()
              : null);
  ...
  public HostForm updateHostForm(Integer id, CreateHostFormDTO request) {
    try {
      ...
      HostForm existingForm =
          hostFormDAO
              .findById(id)
              .orElseThrow(
                  () -> new IllegalArgumentException("Host form not found with id: " + id));
      ...
      existingForm.setStoreName(
          request.getStoreName() != null ? request.getStoreName().trim() : null);
      ...
      // 更新聯絡資訊
      existingForm.setHostContactInformation(
          request.getHostContactInformation() != null
              ? request.getHostContactInformation().trim()
              : null);
      existingForm.setTransferInformation(
          request.getTransferInformation() != null
              ? request.getTransferInformation().trim()
              : null);
  ...
  public void deleteHostForm(Integer id) {
    try {
      ...
      HostForm hostForm =
          hostFormDAO.findById(id).orElseThrow(() -> new IllegalArgumentException("找不到該團購表單"));
